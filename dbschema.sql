--
-- File generated with SQLiteStudio v3.2.1 on Fri Aug 31 10:53:34 2018
--
...
-- Index: asset_longname_idx
CREATE UNIQUE INDEX asset_longname_idx ON assets(asset_longname);

-- Index: backward_status_idx
CREATE INDEX backward_status_idx ON order_matches (backward_asset, status);

-- Index: block_index_idx
CREATE INDEX block_index_idx ON blocks (block_index);

-- Index: block_index_message_index_idx
CREATE INDEX block_index_message_index_idx ON messages (block_index, message_index);

-- Index: cancels_block_index_idx
CREATE INDEX cancels_block_index_idx ON cancels (block_index);

-- Index: contract_id_idx
...
-- Index: feed_valid_bettype_idx
CREATE INDEX feed_valid_bettype_idx ON bets (feed_address, status, bet_type);

-- Index: forward_status_idx
CREATE INDEX forward_status_idx ON order_matches (forward_asset, status);

-- Index: give_asset_idx
CREATE INDEX give_asset_idx ON orders (give_asset);

-- Index: give_get_status_idx
CREATE INDEX give_get_status_idx ON orders (get_asset, give_asset, status);

-- Index: give_status_idx
CREATE INDEX give_status_idx ON orders (give_asset, status);

-- Index: id_idx
CREATE INDEX id_idx ON assets (asset_id);

-- Index: index_hash_idx
CREATE INDEX index_hash_idx ON blocks (block_index, block_hash);

-- Index: index_hash_index_idx
CREATE INDEX index_hash_index_idx ON transactions (tx_index, tx_hash, block_index);

-- Index: index_index_idx
CREATE INDEX index_index_idx ON transactions (block_index, tx_index);

-- Index: match_expire_idx
...
-- Index: rps_match_expire_idx
CREATE INDEX rps_match_expire_idx ON rps_matches (status, match_expire_index);

-- Index: rps_match_id_idx
CREATE INDEX rps_match_id_idx ON rpsresolves (rps_match_id);

-- Index: rps_tx0_address_idx
CREATE INDEX rps_tx0_address_idx ON rps_matches (tx0_address);

-- Index: rps_tx1_address_idx
CREATE INDEX rps_tx1_address_idx ON rps_matches (tx1_address);

-- Index: source_give_status_idx
...
-- Index: status_source_idx
CREATE INDEX status_source_idx ON broadcasts (status, source);

-- Index: status_source_index_idx
CREATE INDEX status_source_index_idx ON broadcasts (status, source, tx_index);

-- Index: timestamp_idx
CREATE INDEX timestamp_idx ON broadcasts (timestamp);

-- Index: tx0_address_idx
CREATE INDEX tx0_address_idx ON order_matches (tx0_address);

-- Index: tx1_address_idx
...
-- Index: valid_asset_idx
CREATE INDEX valid_asset_idx ON issuances (asset, status);

-- Index: valid_feed_idx
CREATE INDEX valid_feed_idx ON bet_matches (feed_address, status);

-- Trigger: _addresses_delete
...
-- Trigger: _assets_insert
CREATE TRIGGER _assets_insert AFTER INSERT ON assets BEGIN
                            INSERT INTO undolog VALUES(NULL, 'DELETE FROM assets WHERE rowid='||new.rowid);
                            END;

-- Trigger: _assets_update
...
-- Trigger: _bet_matches_insert
CREATE TRIGGER _bet_matches_insert AFTER INSERT ON bet_matches BEGIN
                            INSERT INTO undolog VALUES(NULL, 'DELETE FROM bet_matches WHERE rowid='||new.rowid);
                            END;

-- Trigger: _bet_matches_update
...
-- Trigger: _bets_insert
CREATE TRIGGER _bets_insert AFTER INSERT ON bets BEGIN
                            INSERT INTO undolog VALUES(NULL, 'DELETE FROM bets WHERE rowid='||new.rowid);
                            END;

-- Trigger: _bets_update
...
-- Trigger: _broadcasts_delete
CREATE TRIGGER _broadcasts_delete BEFORE DELETE ON broadcasts BEGIN
                            INSERT INTO undolog VALUES(NULL, 'INSERT INTO broadcasts(rowid,tx_index,tx_hash,block_index,source,timestamp,value,fee_fraction_int,text,locked,status) VALUES('||old.rowid||','||quote(old.tx_index)||','||quote(old.tx_hash)||','||quote(old.block_index)||','||quote(old.source)||','||quote(old.timestamp)||','||quote(old.value)||','||quote(old.fee_fraction_int)||','||quote(old.text)||','||quote(old.locked)||','||quote(old.status)||')');
                            END;

-- Trigger: _broadcasts_insert
CREATE TRIGGER _broadcasts_insert AFTER INSERT ON broadcasts BEGIN
                            INSERT INTO undolog VALUES(NULL, 'DELETE FROM broadcasts WHERE rowid='||new.rowid);
                            END;

-- Trigger: _broadcasts_update
CREATE TRIGGER _broadcasts_update AFTER UPDATE ON broadcasts BEGIN
                            INSERT INTO undolog VALUES(NULL, 'UPDATE broadcasts SET tx_index='||quote(old.tx_index)||',tx_hash='||quote(old.tx_hash)||',block_index='||quote(old.block_index)||',source='||quote(old.source)||',timestamp='||quote(old.timestamp)||',value='||quote(old.value)||',fee_fraction_int='||quote(old.fee_fraction_int)||',text='||quote(old.text)||',locked='||quote(old.locked)||',status='||quote(old.status)||' WHERE rowid='||old.rowid);
                            END;

-- Trigger: _gzrpays_delete
CREATE TRIGGER _gzrpays_delete BEFORE DELETE ON gzrpays BEGIN
                            INSERT INTO undolog VALUES(NULL, 'INSERT INTO gzrpays(rowid,tx_index,tx_hash,block_index,source,destination,gzr_amount,order_match_id,status) VALUES('||old.rowid||','||quote(old.tx_index)||','||quote(old.tx_hash)||','||quote(old.block_index)||','||quote(old.source)||','||quote(old.destination)||','||quote(old.gzr_amount)||','||quote(old.order_match_id)||','||quote(old.status)||')');
                            END;

-- Trigger: _gzrpays_insert
CREATE TRIGGER _gzrpays_insert AFTER INSERT ON gzrpays BEGIN
                            INSERT INTO undolog VALUES(NULL, 'DELETE FROM gzrpays WHERE rowid='||new.rowid);
                            END;

-- Trigger: _gzrpays_update
CREATE TRIGGER _gzrpays_update AFTER UPDATE ON gzrpays BEGIN
                            INSERT INTO undolog VALUES(NULL, 'UPDATE gzrpays SET tx_index='||quote(old.tx_index)||',tx_hash='||quote(old.tx_hash)||',block_index='||quote(old.block_index)||',source='||quote(old.source)||',destination='||quote(old.destination)||',gzr_amount='||quote(old.gzr_amount)||',order_match_id='||quote(old.order_match_id)||',status='||quote(old.status)||' WHERE rowid='||old.rowid);
                            END;

-- Trigger: _burns_delete
CREATE TRIGGER _burns_delete BEFORE DELETE ON burns BEGIN
                            INSERT INTO undolog VALUES(NULL, 'INSERT INTO burns(rowid,tx_index,tx_hash,block_index,source,burned,earned,status) VALUES('||old.rowid||','||quote(old.tx_index)||','||quote(old.tx_hash)||','||quote(old.block_index)||','||quote(old.source)||','||quote(old.burned)||','||quote(old.earned)||','||quote(old.status)||')');
                            END;

-- Trigger: _burns_insert
CREATE TRIGGER _burns_insert AFTER INSERT ON burns BEGIN
                            INSERT INTO undolog VALUES(NULL, 'DELETE FROM burns WHERE rowid='||new.rowid);
                            END;

-- Trigger: _burns_update
CREATE TRIGGER _burns_update AFTER UPDATE ON burns BEGIN
                            INSERT INTO undolog VALUES(NULL, 'UPDATE burns SET tx_index='||quote(old.tx_index)||',tx_hash='||quote(old.tx_hash)||',block_index='||quote(old.block_index)||',source='||quote(old.source)||',burned='||quote(old.burned)||',earned='||quote(old.earned)||',status='||quote(old.status)||' WHERE rowid='||old.rowid);
                            END;

-- Trigger: _cancels_delete
CREATE TRIGGER _cancels_delete BEFORE DELETE ON cancels BEGIN
                            INSERT INTO undolog VALUES(NULL, 'INSERT INTO cancels(rowid,tx_index,tx_hash,block_index,source,offer_hash,status) VALUES('||old.rowid||','||quote(old.tx_index)||','||quote(old.tx_hash)||','||quote(old.block_index)||','||quote(old.source)||','||quote(old.offer_hash)||','||quote(old.status)||')');
                            END;

-- Trigger: _cancels_insert
CREATE TRIGGER _cancels_insert AFTER INSERT ON cancels BEGIN
                            INSERT INTO undolog VALUES(NULL, 'DELETE FROM cancels WHERE rowid='||new.rowid);
                            END;

-- Trigger: _cancels_update
CREATE TRIGGER _cancels_update AFTER UPDATE ON cancels BEGIN
                            INSERT INTO undolog VALUES(NULL, 'UPDATE cancels SET tx_index='||quote(old.tx_index)||',tx_hash='||quote(old.tx_hash)||',block_index='||quote(old.block_index)||',source='||quote(old.source)||',offer_hash='||quote(old.offer_hash)||',status='||quote(old.status)||' WHERE rowid='||old.rowid);
                            END;

-- Trigger: _contracts_delete
...
-- Trigger: _issuances_insert
CREATE TRIGGER _issuances_insert AFTER INSERT ON issuances BEGIN
                            INSERT INTO undolog VALUES(NULL, 'DELETE FROM issuances WHERE rowid='||new.rowid);
                            END;

-- Trigger: _issuances_update
...
-- Trigger: _order_matches_insert
CREATE TRIGGER _order_matches_insert AFTER INSERT ON order_matches BEGIN
                            INSERT INTO undolog VALUES(NULL, 'DELETE FROM order_matches WHERE rowid='||new.rowid);
                            END;

-- Trigger: _order_matches_update
...
-- Trigger: _orders_insert
CREATE TRIGGER _orders_insert AFTER INSERT ON orders BEGIN
                            INSERT INTO undolog VALUES(NULL, 'DELETE FROM orders WHERE rowid='||new.rowid);
                            END;

-- Trigger: _orders_update
...
-- Trigger: _rps_expirations_delete
CREATE TRIGGER _rps_expirations_delete BEFORE DELETE ON rps_expirations BEGIN
                            INSERT INTO undolog VALUES(NULL, 'INSERT INTO rps_expirations(rowid,rps_index,rps_hash,source,block_index) VALUES('||old.rowid||','||quote(old.rps_index)||','||quote(old.rps_hash)||','||quote(old.source)||','||quote(old.block_index)||')');
                            END;

-- Trigger: _rps_expirations_insert
CREATE TRIGGER _rps_expirations_insert AFTER INSERT ON rps_expirations BEGIN
                            INSERT INTO undolog VALUES(NULL, 'DELETE FROM rps_expirations WHERE rowid='||new.rowid);
                            END;

-- Trigger: _rps_expirations_update
...
-- Trigger: _rps_matches_insert
CREATE TRIGGER _rps_matches_insert AFTER INSERT ON rps_matches BEGIN
                            INSERT INTO undolog VALUES(NULL, 'DELETE FROM rps_matches WHERE rowid='||new.rowid);
                            END;

-- Trigger: _rps_matches_update
...
-- Trigger: _sends_insert
CREATE TRIGGER _sends_insert AFTER INSERT ON sends BEGIN
                            INSERT INTO undolog VALUES(NULL, 'DELETE FROM sends WHERE rowid='||new.rowid);
                            END;

-- Trigger: _sends_update
